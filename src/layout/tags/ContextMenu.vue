<template>
    <n-dropdown
        :show="show"
        :options="options"
        :x="x"
        :y="y"
        placement="bottom-start"
        @clickoutside="handleHideDropdown"
        @select="handleSelect"
    />
</template>

<script setup>
import { useTagsStore, mainStore } from "@/store";
import Icon from "@/components/icon.vue";

const props = defineProps({
    show: {
        type: Boolean,
        default: false,
    },
    currentPath: {
        type: String,
        default: "",
    },
    x: {
        type: Number,
        default: 0,
    },
    y: {
        type: Number,
        default: 0,
    },
});

const emit = defineEmits(["update:show"]);

const tagsStore = useTagsStore();
const appStore = mainStore();

const options = computed(() => [
    {
        label: "重新加载",
        key: "reload",
        disabled: props.currentPath !== tagsStore.activeTag,
        icon: () => h(Icon, { iconName: "refresh" }),
    },
    {
        label: "关闭",
        key: "close",
        disabled: tagsStore.tags.length <= 1,
        icon: () => h(Icon, { iconName: "close" }),
    },
    {
        label: "关闭其他",
        key: "close-other",
        disabled: tagsStore.tags.length <= 1,
        icon: () => h(Icon, { iconName: "cut" }),
    },
    {
        label: "关闭左侧",
        key: "close-left",
        disabled:
            tagsStore.tags.length <= 1 ||
            props.currentPath === tagsStore.tags[0].path,
        icon: () => h(Icon, { iconName: "leftArrow" }),
    },
    {
        label: "关闭右侧",
        key: "close-right",
        disabled:
            tagsStore.tags.length <= 1 ||
            props.currentPath ===
                tagsStore.tags[tagsStore.tags.length - 1].path,
        icon: () => h(Icon, { iconName: "rightArrow" }),
    },
]);

const route = useRoute();
const actionMap = new Map([
    [
        "reload",
        () => {
            if (route.meta?.keepAlive) {
                // 重置keepAlive
                appStore.setAliveKeys(route.name, +new Date());
            }
            appStore.reloadPage();
        },
    ],
    [
        "close",
        () => {
            tagsStore.removeTag(props.currentPath);
        },
    ],
    [
        "close-other",
        () => {
            tagsStore.removeOther(props.currentPath);
        },
    ],
    [
        "close-left",
        () => {
            tagsStore.removeLeft(props.currentPath);
        },
    ],
    [
        "close-right",
        () => {
            tagsStore.removeRight(props.currentPath);
        },
    ],
]);

function handleHideDropdown() {
    emit("update:show", false);
}

function handleSelect(key) {
    const actionFn = actionMap.get(key);
    actionFn && actionFn();
    handleHideDropdown();
}
</script>
